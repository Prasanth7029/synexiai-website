...
async function gatherFiles() {
  ...
  for (const pattern of patterns) {
    const matches = await glob(pattern, { withFileTypes: false, dot: false, nodir: true });
...
async function run() {
  ...
  const outDir = join(__dirname, '../netlify/functions/chat-assistant/knowledge');
  ...
  await writeFile(join(outDir, 'site-map.json'), JSON.stringify(payload, null, 2), 'utf8');
