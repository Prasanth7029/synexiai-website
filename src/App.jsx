  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (!cancelled) { AOS.init({ duration: 1000, easing: 'ease-out', once: true, mirror: false }); AOS.refresh(); }
      } catch {}
    };
    if ('requestIdleCallback' in window) window.requestIdleCallback(() => init());
    else { const t = setTimeout(init, 1200); return () => clearTimeout(t); }
        ...
        <Suspense fallback={<LoaderScreen />}>
          <Layout>
            <Routes>
              ...
              <Route path="/projects"      element={<ProjectsPage />} />   {/* curated showcase */}
              <Route path="/portfolio"     element={<PortfolioPage />} />  {/* GitHub + roadmap */}
