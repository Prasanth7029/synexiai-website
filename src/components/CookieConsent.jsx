...
export default function CookieConsent() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50">
      <div className="mx-auto max-w-4xl m-4 rounded-2xl p-4 shadow-lg
        ...
        <p className="text-sm mb-3">
          ...
          See our <a className="underline" href="/cookie-policy">Cookie Policy</a>.
        </p>
        <div className="flex gap-3">
          <button
            onClick={() => { localStorage.setItem(STORAGE_KEY, "accepted"); setChoice("accepted"); }}
            className="px-4 py-2 rounded-xl bg-cyan-500 text-black font-medium"
          >
            Accept all
          </button>
          <button
            onClick={() => { localStorage.setItem(STORAGE_KEY, "rejected"); setChoice("rejected"); }}
            className="px-4 py-2 rounded-xl border border-white/20"
          >
            Reject non-essential
          </button>
          <a href="/cookie-policy" className="px-4 py-2 text-sm underline self-center">
            Learn more
          </a>
        </div>
      </div>
